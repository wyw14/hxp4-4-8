...
void main() {
    ...
    staticColor += vec3((hash(uv + vec2(u_time * 45.0, 0.0)) - 0.5) * 0.1);
    ...
    if (u_vhsTint > 0.0) {
        float r = fbm(uv * vec2(30.0, 20.0) + vec2(u_time * 0.5, u_time * 0.5));
        float g = fbm(uv * vec2(30.0, 20.0) + vec2(u_time * 0.5 + 100.0, u_time * 0.5 + 100.0));
        float b = fbm(uv * vec2(30.0, 20.0) + vec2(u_time * 0.5 + 200.0, u_time * 0.5 + 200.0));
    ...
    finalColor += vec3((hash(uv * 1000.0 + u_time * 500.0) - 0.5) * 0.02 * u_staticAmount);
    
    vec2 crtCurve = abs(uv - vec2(0.5));
